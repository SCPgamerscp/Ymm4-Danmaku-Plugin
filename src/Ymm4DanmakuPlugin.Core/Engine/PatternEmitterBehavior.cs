...
public sealed class PatternEmitterBehavior(EmitterSettings settings) : IEmitterBehavior
{
    ...
    private const int MaxShotsPerStep = 16384;
        ...
        double stackSpeedStep)
    {
        switch (pattern.Kind)
        {
            ...
            case PatternKind.Spiral:
            {
                ...
                var wallStep = way > 1 ? wallWidth / (way - 1) : 0;
                var wallOffset = wallWidth != 0 ? Vec2.FromDegrees(baseAngle + 90, -wallWidth / 2 + wallStep * index) : Vec2.Zero;
                return (dir, streamOffset + wallOffset, 0);
            }
            ...
            case PatternKind.Scatter:
            {
                ...
                var wallStep = way > 1 ? wallWidth / (way - 1) : 0;
                var wallOffset = wallWidth != 0 ? Vec2.FromDegrees(baseAngle + 90, -wallWidth / 2 + wallStep * index) : Vec2.Zero;
                return (angle, streamOffset + wallOffset, 0);
            }

            case PatternKind.Wall:
            {
                var width = wallWidth != 0 ? wallWidth : 1280;
            ...
            case PatternKind.Rose:
            {
                ...
                var wallStep = way > 1 ? wallWidth / (way - 1) : 0;
                var wallOffset = wallWidth != 0 ? Vec2.FromDegrees(baseAngle + 90, -wallWidth / 2 + wallStep * index) : Vec2.Zero;
                return (angle, streamOffset + wallOffset, extraSpeed);
            }

            case PatternKind.Laser:
            {
                var spacing = laserSpacing != 0 ? laserSpacing : 24;
                var distance = spawnRadius + spacing * index;
                var wallStep = way > 1 ? wallWidth / (way - 1) : 0;
                var wallOffset = wallWidth != 0 ? Vec2.FromDegrees(baseAngle + 90, -wallWidth / 2 + wallStep * index) : Vec2.Zero;
                return (baseAngle, Vec2.FromDegrees(baseAngle, distance) + wallOffset, 0);
            }
